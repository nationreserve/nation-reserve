import{z}from"zod";
const envBoolean=z.union([z.boolean(),z.enum(["true","false","1","0"]).transform(v=>v==="true"||v==="1")]);
export const paymentConfigSchema=z.object({
  ...
  PAYOUT_ONBOARDING_REFRESH_URL:z.string().url().default("http://localhost:5173/settings/payouts"),PAYMENT_REQUEST_TIMEOUT_MS:z.coerce.number().int().positive().default(15000),
  PAYMENT_RECONCILIATION_INTERVAL_SECONDS:z.coerce.number().int().positive().default(300),
  ...
  COMPANY_AUTOPAY_ENABLED_DEFAULT:envBoolean.default(false),COMPANY_PAYMENT_RETRY_LIMIT:z.coerce.number().int().nonnegative().default(3),
  COMPANY_PAYMENT_RETRY_INTERVAL_HOURS:z.coerce.number().int().positive().default(24),
  OWNER_MINIMUM_PAYOUT_MINOR_UNITS:z.coerce.number().int().positive().default(2500),
  OWNER_PAYOUT_FREQUENCY:z.enum(["weekly","semimonthly","monthly","manual"]).default("weekly"),
  ...
  if(v.NODE_ENV!=="production"&&(v.PAYMENT_PROVIDER_API_KEY??v.STRIPE_SECRET_KEY)?.startsWith("sk_live_"))c.addIssue({code:"custom",message:"Stripe live key rejected outside production"});
  for(const url of [v.PAYMENT_METHOD_SETUP_RETURN_URL,v.PAYOUT_ONBOARDING_RETURN_URL,v.PAYOUT_ONBOARDING_REFRESH_URL])
    if(v.NODE_ENV==="production"&&!url.startsWith("https://"))c.addIssue({code:"custom",message:"Production return URLs require HTTPS"});
});
