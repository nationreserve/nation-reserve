...
const apiEnvironmentSchema = z
  .object({
    ...
    S3_TRAINING_DATA_BUCKET: z.string().min(3),
    S3_MANUFACTURER_DOCUMENTS_BUCKET: z.string().min(3),
    S3_CONTRACT_DOCUMENTS_BUCKET: z.string().min(3),
  ...
  .superRefine((value, ctx) => {
    ...
    if (!value.COOKIE_SECURE)
      issue(
        "COOKIE_SECURE",
        "Secure cookies are mandatory in optimized cloud runtimes.",
      );
    if (!value.TIMELINE_PROJECTION_ENABLED)
      issue(
        "TIMELINE_PROJECTION_ENABLED",
        "Mandatory Appendix O projection cannot be disabled.",
      );
    if (!value.WEB_ORIGIN.startsWith("https://"))
      issue("WEB_ORIGIN", "Cloud runtime origin must use HTTPS.");
    if (!value.S3_ENDPOINT.startsWith("https://"))
      issue("S3_ENDPOINT", "Cloud runtime storage endpoint must use HTTPS.");
    if (value.DEPLOY_ENVIRONMENT !== "production") return;
    if (value.PAYMENT_PROVIDER !== "stripe")
      issue("PAYMENT_PROVIDER", "Fake payment provider is forbidden in production.");
    if (value.PAYMENT_PROVIDER_ENVIRONMENT !== "live")
      issue(
        "PAYMENT_PROVIDER_ENVIRONMENT",
        "Production requires the explicitly approved live provider environment.",
      );
    if (!value.PAYMENT_EXECUTION_ENABLED)
      issue(
        "PAYMENT_EXECUTION_ENABLED",
        "Production payment execution must be explicitly enabled after approval.",
      );
  });
