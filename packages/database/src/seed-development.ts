...
const sandboxCredential = "rwp_sbx_111111111111_AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
const productionCredential = "rwp_prod_222222222222_BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB";
const transferCode = "RWP-DEVELOPMENT-TRANSFER-CODE-ONLY";

export async function seedDevelopmentIdentities(pool: Pool) {
  if (process.env.NODE_ENV === "production") throw new Error("Development identities cannot be seeded in production.");
  const auth = parseAuthEnvironment({ ...process.env, NODE_ENV: "development" });
  const integration = parseIntegrationEnvironment({ ...process.env, NODE_ENV: "development" });
  const passwordHash = await hashPassword(developmentPassword, auth);
  await pool.query("BEGIN");
  try {
    const identities = [
      ["00000000-0000-4000-8000-000000000101","platform.admin@roboworkpool.test","Platform Administrator",
        "00000000-0000-4000-8000-000000000001","platform_admin"],
      ["00000000-0000-4000-8000-000000000102","owner@roboworkpool.test","Development Robot Owner",
        "00000000-0000-4000-8000-000000000201","owner"],
      ["00000000-0000-4000-8000-000000000103","company.admin@roboworkpool.test","Hiring Company Administrator",
        "00000000-0000-4000-8000-000000000202","administrator"],
      ["00000000-0000-4000-8000-000000000104","manufacturer.admin@roboworkpool.test","Manufacturer Administrator",
        "00000000-0000-4000-8000-000000000203","administrator"],
    ] as const;
    ...
    for (const [id,email,name,organizationId,role] of identities) {
      await pool.query(`INSERT INTO users(id,email,email_normalized,display_name,status,email_verified_at)
        VALUES($1,$2,$2,$3,'active',now()) ON CONFLICT(id) DO NOTHING`, [id,email,name]);
      await pool.query(`INSERT INTO user_credentials(user_id,password_hash,password_algorithm)
        VALUES($1,$2,'argon2id') ON CONFLICT(user_id) DO UPDATE SET password_hash=EXCLUDED.password_hash`,
      [id,passwordHash]);
      await pool.query(`INSERT INTO organization_memberships(organization_id,user_id,role,status)
        VALUES($1,$2,$3,'active') ON CONFLICT DO NOTHING`, [organizationId,id,role]);
    }
    ...
    await pool.query(`INSERT INTO manufacturers(id,organization_id,approval_status,production_access_status,
      ...
      'approved','enabled',now(),now(),'production_enabled') ON CONFLICT(id) DO NOTHING`);
    await pool.query(`INSERT INTO manufacturer_api_credentials(id,manufacturer_id,environment,credential_name,
