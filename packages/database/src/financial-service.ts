...
import {
  calculateFinancialAllocation,
  type FinancialConfig,
} from "@nation-reserve/financial-domain";
...
export class FinancialFinalizationService {
  ...
  async finalizeReadyIntervals(limit = this.config.FINANCIAL_FINALIZATION_BATCH_SIZE) {
    const candidates = await this.pool.query<{ id: string }>(
      `SELECT i.id FROM verified_operating_intervals i
      ...
      AND EXISTS(SELECT 1 FROM contract_payment_integrity_states p WHERE p.contract_id=i.contract_id
        AND p.state IN ('funded','active','resolved_reactivated'))
      AND NOT EXISTS(SELECT 1 FROM operating_time_holds h WHERE h.interval_id=i.id AND h.status='active')
      ORDER BY i.interval_end_at LIMIT $1`,
      [limit],
    );
    const results = [];
    for (const row of candidates.rows)
      results.push(await this.finalizeInterval(row.id));
    return results;
  }
  async finalizeInterval(intervalId: string) {
    ...
    try {
      ...
      const interval = (
        await client.query(
          `SELECT i.*,f.base_rate_minor_units_per_hour,
        ...
        AND i.verified_duration_seconds>0 AND i.review_status IN ('not_required','approved')
        AND EXISTS(SELECT 1 FROM contract_payment_integrity_states p WHERE p.contract_id=i.contract_id
          AND p.state IN ('funded','active','resolved_reactivated')) FOR UPDATE`,
          [intervalId],
        )
      ).rows[0];
      if (!interval) throw new Error("FINANCIAL_INTERVAL_NOT_READY");
      const existing = await client.query(
        `SELECT * FROM financial_accruals
        WHERE verified_operating_interval_id=$1 AND status='posted'`,
        [intervalId],
      );
      if (existing.rows[0]) {
      ...
      const held = await client.query(
        `SELECT 1 FROM operating_time_holds WHERE interval_id=$1 AND status='active'
        UNION ALL SELECT 1 FROM financial_holds WHERE scope_id=$1 AND status='active' LIMIT 1`,
        [intervalId],
      );
      if (held.rows[0]) throw new Error("FINANCIAL_INTERVAL_HELD");
      const period = (
        await client.query(
          `SELECT id FROM financial_periods WHERE status IN ('open','reopened')
      ...
      const amounts = calculateFinancialAllocation({
        verifiedDurationSeconds: interval.verified_duration_seconds,
        ...
        companyPlatformFeeBasisPoints: interval.company_platform_fee_basis_points,
        currency: "USD",
      ...
      await client.query(
        `INSERT INTO journal_entries(id,journal_number,entry_type,status,effective_at,
        ...
        'Verified robot operating-time accrual','USD',$6)`,
        [
          journalId,
          `RWP-JRN-${Date.now()}-${accrualId.slice(0, 8)}`,
          interval.interval_end_at,
          period.id,
          intervalId,
          crypto.randomUUID(),
        ],
      ...
      const accountRows = await client.query(
        `SELECT id,account_code FROM financial_accounts
        WHERE account_code=ANY($1::text[])`,
        [["1000", "2000", "4000", "4010"]],
      );
      const accounts = new Map(
        accountRows.rows.map((row) => [row.account_code, row.id]),
      );
      const lines = [
        [
          accounts.get("1000"),
          amounts.companyTotalChargeMinorUnits,
          0,
          "Company receivable",
        ],
        [
          accounts.get("2000"),
          0,
          amounts.ownerNetEarningMinorUnits,
          "Robot Owner payable",
        ],
        [
          accounts.get("4000"),
          0,
          amounts.companyPlatformFeeMinorUnits,
          "Company platform fee",
        ],
        [
          accounts.get("4010"),
          0,
          amounts.ownerPlatformFeeMinorUnits,
          "Owner platform fee",
        ],
      ];
      let number = 1;
      for (const [accountId, debit, credit, description] of lines)
        await client.query(
          `INSERT INTO journal_lines(journal_entry_id,financial_account_id,line_number,
          ...
          [
            ...
            credit,
            description,
            interval.hiring_company_id,
            interval.robot_id,
            interval.contract_id,
            interval.assignment_id,
            intervalId,
            interval.financial_configuration_version_id,
          ],
        );
      const values = [
        accrualId,
        ...
        period.id,
        interval.financial_configuration_version_id,
        this.config.FINANCIAL_CALCULATION_VERSION,
        interval.robot_id,
        interval.robot_owner_organization_id,
        interval.manufacturer_id,
        interval.hiring_company_id,
        interval.contract_id,
        interval.contract_version_id,
        interval.assignment_id,
        interval.facility_id,
        interval.department_id,
        interval.verified_duration_seconds,
        amounts.companyBaseChargeMinorUnits,
        amounts.companyPlatformFeeMinorUnits,
        amounts.companyTotalChargeMinorUnits,
        amounts.ownerGrossEarningMinorUnits,
        amounts.ownerPlatformFeeMinorUnits,
        amounts.ownerNetEarningMinorUnits,
        amounts.platformRevenueMinorUnits,
        amounts.roundingAdjustmentMinorUnits,
        journalId,
      ];
      const accrual = (
        await client.query(
          `INSERT INTO financial_accruals(id,verified_operating_interval_id,
      ...
      await client.query(
        `UPDATE financial_accruals SET status='posted',updated_at=now() WHERE id=$1`,
        [accrualId],
      );
      await client.query(
        `UPDATE verified_operating_intervals SET status='finalized',
