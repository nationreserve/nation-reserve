...
describe("Prompt 015 accessibility and permission boundaries",()=>{
  ...
  it("does not expose availability mutation while its policy API is absent",()=>{render(<OwnerPage path="/owner/robots/example"/>);expect(screen.queryByRole("button",{name:/mark.*available|mark.*unavailable/i})).not.toBeInTheDocument();expect(screen.getByText("Availability is policy controlled")).toBeInTheDocument()});
