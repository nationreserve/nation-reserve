/* eslint-disable @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-base-to-string */
import {
  ActivationService,
...
export class PostgresIntegrationRouteService implements IntegrationRouteService {
  ...
  async authenticateCredential(raw: string): Promise<IntegrationPrincipal> {
    ...
    const result = await this.pool.query(
      `SELECT c.id,c.manufacturer_id,c.environment,c.secret_hash,
    ...
    if (
      !row ||
      !constantTimeHashEqual(supplied, row.secret_hash) ||
      !["active", "rotating"].includes(row.status) ||
      row.expires_at <= new Date() ||
      row.environment !== parsed.environment ||
      row.suspended_at ||
      (row.environment === "production" &&
        row.integration_status !== "production_enabled")
    ) {
      throw denied("MANUFACTURER_AUTHENTICATION_FAILED", 401);
    }
    await this.pool.query(
      "UPDATE manufacturer_api_credentials SET last_used_at=now() WHERE id=$1",
      [row.id],
    );
    return {
      ...
      scopes: row.scopes as ManufacturerScope[],
    };
  }
  async application(userId: string, organizationId: string) {
    ...
    return (
      (
        await this.pool.query(
          `SELECT * FROM manufacturer_applications
      WHERE manufacturer_id=$1 AND is_current`,
          [manufacturer.id],
        )
      ).rows[0] ?? {}
    );
  ...
  async saveApplication(userId: string, organizationId: string, input: object) {
    ...
    const result = await this.pool.query(
      `INSERT INTO manufacturer_applications
      ...
      [
        ...
        value.operationsContactName,
        value.operationsContactEmail,
        value.primaryCountryCode,
        value.businessDescription,
        JSON.stringify(value.robotCategories ?? []),
        value.anticipatedRobotVolume,
        JSON.stringify(value.integrationReadiness ?? {}),
  ...
  async submitApplication(userId: string, organizationId: string) {
    ...
    await this.businessTransaction(async (client) => {
      const result = await client.query(
        `UPDATE manufacturer_applications SET status='submitted',
      ...
      await event(
        client,
        userId,
        "manufacturer.application.submitted",
        "manufacturer",
        manufacturer.id,
  ...
  async reviewApplication(
    ...
    if (["rejected", "information_requested"].includes(action) && !reason)
      throw denied("DECISION_REASON_REQUIRED", 400);
    await this.businessTransaction(async (client) => {
      const found = await client.query(
        `SELECT a.*,m.sandbox_approved_at FROM manufacturer_applications a
        JOIN manufacturers m ON m.id=a.manufacturer_id WHERE a.id=$1 FOR UPDATE`,
        [applicationId],
      );
      const row = found.rows[0];
      if (!row) throw denied("NOT_FOUND", 404);
      if (action === "production_approved" && !row.sandbox_approved_at)
        throw denied("SANDBOX_APPROVAL_REQUIRED", 409);
      await client.query(
        `UPDATE manufacturer_applications SET status=$2,reviewed_at=now(),
        reviewed_by_user_id=$3,decision_reason=$4 WHERE id=$1`,
        [applicationId, action, userId, reason],
      );
      if (action === "sandbox_approved")
        await client.query(
          `UPDATE manufacturers SET
        sandbox_approved_at=now(),approval_status='sandbox_approved',production_access_status='sandbox',
        integration_status='sandbox_testing'
        WHERE id=$1`,
          [row.manufacturer_id],
        );
      if (action === "production_approved")
        await client.query(
          `UPDATE manufacturers SET
        production_approved_at=now(),approval_status='production_approved',production_access_status='production',
        integration_status='production_enabled' WHERE id=$1`,
          [row.manufacturer_id],
        );
      await event(
        client,
        userId,
        `manufacturer.application.${action}`,
        "manufacturer",
        row.manufacturer_id,
  ...
  async credentials(userId: string, organizationId: string) {
    ...
    return (
      await this.pool.query(
        `SELECT id,environment,credential_name,credential_prefix,status,
  ...
  async rotateCredential(userId: string, organizationId: string, credentialId: string) {
    ...
    const prior = (
      await this.pool.query(
        `SELECT * FROM manufacturer_api_credentials
      WHERE id=$1 AND manufacturer_id=$2 AND status='active'`,
        [credentialId, manufacturer.id],
      )
    ).rows[0];
    if (!prior) throw denied("NOT_FOUND", 404);
    const created = await this.#credentials.issue(
      userId,
      manufacturer.id,
      prior.environment,
      `${prior.credential_name} replacement`,
      prior.scopes,
    );
    await this.pool.query(
      `UPDATE manufacturer_api_credentials SET status='rotating',rotated_at=now(),
  ...
  async revokeCredential(
    ...
    const manufacturer = await this.manufacturer(userId, organizationId, true);
    await this.pool.query(
      `UPDATE manufacturer_api_credentials SET status='revoked',revoked_at=now(),
      revocation_reason=$3 WHERE id=$1 AND manufacturer_id=$2`,
      [credentialId, manufacturer.id, reason],
    );
  }
  async models(userId: string, organizationId: string) {
    ...
    return (
      await this.pool.query(
        `SELECT m.*,COALESCE(json_agg(r ORDER BY r.revision_number)
      ...
      const model = (
        ...
        VALUES($1,$2,$3,$4,$5,$6,'draft',$7,$8,$9) RETURNING *`,
          [
            ...
            JSON.stringify(value.supportedApiVersions ?? []),
            JSON.stringify(value.operationalStateMapping ?? {}),
          ],
        )
      ).rows[0];
      await client.query(
  ...
  async submitModel(userId: string, organizationId: string, modelId: string) {
    const manufacturer = await this.manufacturer(userId, organizationId, true);
    await this.pool.query(
      `UPDATE robot_models SET approval_status='submitted',submitted_at=now()
      WHERE id=$1 AND manufacturer_id=$2 AND approval_status='draft'`,
      [modelId, manufacturer.id],
    );
    await this.pool.query(
      `UPDATE robot_model_revisions SET status='submitted',submitted_at=now()
      WHERE robot_model_id=$1 AND status='draft'`,
      [modelId],
    );
  }
  async reviewModel(userId: string, modelId: string, action: string, reason?: string) {
    ...
    await this.businessTransaction(async (client) => {
      await client.query(
        `UPDATE robot_models SET approval_status=$2,reviewed_at=now(),
        reviewed_by_user_id=$3,decision_reason=$4 WHERE id=$1`,
        [modelId, action, userId, reason],
      );
      await client.query(
        `UPDATE robot_model_revisions SET status=$2,reviewed_at=now()
        WHERE robot_model_id=$1 AND status IN ('submitted','under_review')`,
        [modelId, action],
      );
      await event(client, userId, `robot.model.${action}`, "robot_model", modelId, {
        reason,
      });
    });
  }
  ...
  async registration(principal: IntegrationPrincipal, id: string) {
    const row = (
      await this.pool.query(
        `SELECT * FROM robot_registration_requests
      WHERE id=$1 AND manufacturer_id=$2 AND environment=$3`,
        [id, principal.manufacturerId, principal.environment],
      )
    ).rows[0];
    if (!row) throw denied("NOT_FOUND", 404);
    return row;
  ...
  async integrationRobot(principal: IntegrationPrincipal, id: string) {
    const row = (
      await this.pool.query(
        `SELECT id,manufacturer_id,robot_model_id,manufacturer_serial_number,
  ...
  async manufacturerRobots(userId: string, organizationId: string) {
    ...
    return (
      await this.pool.query(
        `SELECT r.*,m.model_name FROM robots r JOIN robot_models m ON m.id=r.robot_model_id
      WHERE r.manufacturer_id=$1 ORDER BY r.created_at DESC`,
        [manufacturer.id],
      )
    ).rows;
  }
  async robot(userId: string, robotId: string) {
    const row = (
      await this.pool.query(
        `SELECT r.*,m.model_name,o.owner_organization_id FROM robots r
      JOIN robot_models m ON m.id=r.robot_model_id LEFT JOIN robot_ownership_records o ON o.robot_id=r.id
      AND o.ownership_status='verified' AND o.ownership_end_at IS NULL WHERE r.id=$1`,
        [robotId],
      )
    ).rows[0];
    if (!row) throw denied("NOT_FOUND", 404);
    ...
    if (!allowed.rowCount) throw denied("PERMISSION_DENIED", 403);
    return row;
  }
  async createTransferCode(userId: string, organizationId: string, robotId: string) {
    ...
    const robot = (
      await this.pool.query(
        "SELECT id FROM robots WHERE id=$1 AND manufacturer_id=$2",
        [robotId, manufacturer.id],
      )
    ).rows[0];
    if (!robot) throw denied("NOT_FOUND", 404);
    ...
    const row = (
      ...
      VALUES($1,$2,$3,now()+make_interval(secs=>$4),$5) RETURNING id,expires_at`,
        [
          robotId,
          manufacturer.id,
          hashIntegrationSecret(code, this.config.apiKeyPepper),
          this.config.transferCodeTtlSeconds,
          userId,
        ],
      )
    ).rows[0];
  ...
  startActivation(principal: IntegrationPrincipal, robotId: string) {
    return this.#activation.start(principal, robotId);
  }
  testActivation(principal: IntegrationPrincipal, id: string, input: object) {
    ...
    return this.#activation.test(principal, id, {
      ...v,
      timestamp: new Date(v.timestamp),
    });
  }
  completeActivation(principal: IntegrationPrincipal, id: string) {
    return this.#activation.complete(principal, id);
  }
  async platformList(userId: string, resource: string) {
    ...
    const table: Record<string, string> = {
      "manufacturers/applications": "manufacturer_applications",
      "robot-models": "robot_models",
      "robot-registrations": "robot_registration_requests",
      "ownership-claims": "robot_ownership_claims",
      activations: "robot_activation_sessions",
    };
    return (
      await this.pool.query(
        `SELECT * FROM ${table[resource]} ORDER BY created_at DESC LIMIT 200`,
      )
    ).rows;
  }
  ...
  private async platform(userId: string, roles: string[]) {
    const row = (
      await this.pool.query(
        `SELECT 1 FROM organization_memberships om JOIN organizations o
      ON o.id=om.organization_id WHERE om.user_id=$1 AND o.organization_type='platform'
      AND om.status='active' AND om.role=ANY($2)`,
        [userId, roles],
      )
    ).rows[0];
...
async function event(
  ...
  await client.query(
    ...
    [actorId, type, aggregateType, aggregateId, payload],
  );
  await client.query(
