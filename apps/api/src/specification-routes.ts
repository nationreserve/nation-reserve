...
export function registerSpecificationRoutes(app:AppFastifyInstance,options:{service:PostgresSpecificationService;authenticate(r:AppFastifyRequest):Promise<{userId:string}>}){const actor=async(r:AppFastifyRequest)=>(await options.authenticate(r)).userId,base="/api/v1/platform/specification";
